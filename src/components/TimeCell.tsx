...
export function TimeCell({ tz, instant, timeClassName, iconClassName, isBusinessTime }: TimeCellProps) {
  ...
  const iconClasses = `${iconClassName || "h-4 w-4 sm:h-5 sm:w-5 -mt-px"} ${isBusinessTime ? "text-primary" : ""}`;
  ...
  return (
    <div className="flex items-center justify-center gap-2">
      ...
      <div className="flex flex-col items-center">
        <h2 className={`${timeClassName || "font-mono tabular-nums leading-none text-xl sm:text-2xl font-semibold"} ${isBusinessTime ? "text-accent-foreground" : ""}`}>
          {time}
        </h2>
        {weekend && (
